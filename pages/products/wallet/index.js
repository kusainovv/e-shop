...
const ProductsPage = ({ products, title }) => {
  const [productsResult, setProductsResult] = useState(products.filter(product => product.type === 'кошелек'));
          ...
          onChange={(e) => {
            if (!e.target.value.length) {
              axios.get(`http://90.156.225.217:3000`).then((r) => {
                setProductsResult(r.data[0].filter(product => product.type === 'кошелек'));
              });
            } else {
              axios
